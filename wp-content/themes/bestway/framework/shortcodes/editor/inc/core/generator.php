...
class mom_su_Generator {
	...
	public static function button( $args = array() ) {
		...
		$args = wp_parse_args( $args, array(
				'target'    => $target,
				'text'      => __( 'Shortcodes', 'theme' ),
				'class'     => 'button',
				'icon'      => 'momizat-icon-embed2',
				'echo'      => true,
				'shortcode' => false
			) );
		// Prepare icon
		if ( $args['icon'] ) $args['icon'] = '<i class="' . $args['icon'] . '" ></i> ';
		// Print button
		...
		// Print/return result
		if ( $args['echo'] ) echo balanceTags($button);
		return $button;
	}
	...
	public static function popup() {
		...
		if ( $output && mom_su_ENABLE_CACHE ) echo balanceTags($output);
		// Cache not found
		else {
			ob_start();
			$tools = apply_filters( 'mom_su/generator/tools', array(
					'<a href="' . admin_url( 'admin.php?page=mom-shortcodes-ultimate' ) . '#tab-1" target="_blank" title="' . __( 'Settings', 'theme' ) . '">' . __( 'Plugin settings', 'theme' ) . '</a>',
					'<a href="http://gndev.info/mom-shortcodes-ultimate/" target="_blank" title="' . __( 'Plugin homepage', 'theme' ) . '">' . __( 'Plugin homepage', 'theme' ) . '</a>',
					'<a href="http://wordpress.org/support/plugin/mom-shortcodes-ultimate/" target="_blank" title="' . __( 'Support forums', 'theme' ) . '">' . __( 'Support forums', 'theme' ) . '</a>'
				) );

			// Add add-ons links
			if ( !mom_su_addon_active( 'maker' ) || !mom_su_addon_active( 'skins' ) || !mom_su_addon_active( 'extra' ) ) $tools[] = '<a href="' . admin_url( 'admin.php?page=mom-shortcodes-ultimate-addons' ) . '" target="_blank" title="' . __( 'Add-ons', 'theme' ) . '" class="mom-su-add-ons">' . __( 'Add-ons', 'theme' ) . '</a>';
?>
		<div id="mom-su-generator-wrap" style="display:none">
			<div id="mom-su-generator">
				<div id="mom-su-generator-header">
					<input type="text" name="mom_su_generator_search" id="mom-su-generator-search" value="" placeholder="<?php _e( 'Search for shortcodes', 'theme' ); ?>" />
					<div id="mom-su-generator-filter">
						<strong><?php _e( 'Filter by type', 'theme' ); ?></strong>
...
?>
	...
	public static function settings() {
		self::access();
		// Param check
		if ( empty( $_REQUEST['shortcode'] ) ) wp_die( __( 'Shortcode not specified', 'theme' ) );
		// Get cache
		...
		// Cache not found
			...
			$actions = apply_filters( 'mom_su/generator/actions', array(
					'insert' => '<a href="javascript:void(0);" class="button button-primary button-large mom-su-generator-insert" data-type="'.$shortcode['type'].'"><i class="fa fa-check"></i> ' . __( 'Insert shortcode', 'theme' ) . '</a>',
					//'preview' => '<a href="javascript:void(0);" class="button button-large mom-su-generator-toggle-preview"><i class="fa fa-eye"></i> ' . __( 'Live preview', 'theme' ) . '</a>'
				) );
			// Shortcode header
			$return = '<div id="mom-su-generator-breadcrumbs">';
			$return .= apply_filters( 'mom_su/generator/breadcrumbs', '<a href="javascript:void(0);" class="mom-su-generator-home" title="' . __( 'Click to return to the shortcodes list', 'theme' ) . '">' . __( 'All shortcodes', 'theme' ) . '</a> &rarr; <span>' . $shortcode['name'] . '</span> <small class="alignright">' . $shortcode['desc'] . '</small><div class="mom-su-generator-clear"></div>' );
			$return .= '</div>';
			// Shortcode note
			if ( isset( $shortcode['note'] ) || isset( $shortcode['example'] ) ) {
				...
				if ( isset( $shortcode['example'] ) ) $return .= wpautop( '<a href="' . admin_url( 'admin.php?page=mom-shortcodes-ultimate-examples&example=' . $shortcode['example'] ) . '" target="_blank">' . __( 'Examples of use', 'theme' ) . ' &rarr;</a>' );
				$return .= '</div></div>';
			}
			// Shortcode has atts
			if ( count( $shortcode['atts'] ) && $shortcode['atts'] ) {
				...
				foreach ( $shortcode['atts'] as $attr_name => $attr_info ) {
					...
					$required = '';
					$required_class = 'mom-su-parent-attr';
					$required_level = '';
					if (isset($attr_info['required'])) {
						$op = isset($attr_info['required'][2]) ? $attr_info['required'][2] : '=';
						$required .= 'data-required="'.$attr_info['required'][0].'" data-value="'.$attr_info['required'][1].'" data-operator="'.$op.'"';
						$required_class = 'require-another-attr';
						$required_level = isset($attr_info['required'][3]) ? ' mom-attr-level-'.$attr_info['required'][3]: '';

					}
					$return .= '<div class="mom-su-generator-attr-container' . $skip . ' '.$required_class.$required_level. '" data-default="' . esc_attr( $default ) . '" '.$required.' >';
					...
					if ( isset( $attr_info['desc'] ) ) $return .= '<div class="mom-su-generator-attr-desc">' . str_replace( array( '<b%value>', '<b_>' ), '<b class="mom-su-generator-set-value" title="' . __( 'Click to set this value', 'theme' ) . '">', $attr_info['desc'] ) . '</div>';
					$return .= '<div class="mom-su-generator-clear"></div></div>';
				}
			}
			...
			// Wrapping shortcode
			else $return .= '<div class="mom-su-generator-attr-container"><h5>' . __( 'Content', 'theme' ) . '</h5><textarea name="mom-su-generator-content" id="mom-su-generator-content" rows="5">' . esc_attr( str_replace( array( '%prefix_', '__' ), mom_su_cmpt(), $shortcode['content'] ) ) . '</textarea></div>';
...
<div class="mom-su-generator-presets alignright" data-shortcode="<?php echo sanitize_key( $_REQUEST['shortcode'] ); ?>">
	<a href="javascript:void(0);" class="button button-large mom-su-gp-button"><i class="fa fa-bars"></i> <?php _e( 'Presets', 'theme' ); ?></a>
	<div class="mom-su-gp-popup">
		<div class="mom-su-gp-head">
			<a href="javascript:void(0);" class="button button-small button-primary mom-su-gp-new"><?php _e( 'Save current settings as preset', 'theme' ); ?></a>
		</div>
	...
	public static function presets_list( $shortcode = false ) {
		...
		if ( is_array( $presets ) && count( $presets ) ) {
			...
			// Hide default text
			echo sprintf( '<b style="display:none">%s</b>', __( 'Presets not found', 'theme' ) );
		}
		// Presets doesn't found
		else echo sprintf( '<b>%s</b>', __( 'Presets not found', 'theme' ) );
	}
