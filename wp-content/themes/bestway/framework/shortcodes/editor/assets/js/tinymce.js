! function () {
	tinymce.create('tinymce.plugins.shortcodesultimate', {
		...
		_html2shortcodes: function (content) {
			...
			content = content.replace(/<div.*?class=\"mom-su-row (.*?)\".*?>(.*?)<\/div>/gi, '[row class="$1"]$2[/column]');
			content = content.replace(/<div.*?class=\"mom-su-column mom-su-column-size-(.*?) (.*?)\".*?>(.*?)<\/div>/gi, '[column size="$1" class="$2"]$3[/column]');
			return content;
			// function t(t, n) {
			// e = e.replace(t, n);
			// }
		...
		_shortcodes2html: function (content) {
			...
			return content;
			// return e = tinymce.trim(e),
			// t(/\[column.*?size="(.*?)".*?class="(.*?)".*?\](.*?)\[\/column\]/gi, '<div class="mom-su-column mom-su-column-size-$1 $2">$3</div>'),
			// t(/\n/gi, "<br />"),
			// t(/\[b\]/gi, "<strong>"),
			// t(/\[\/b\]/gi, "</strong>"),
			// t(/\[i\]/gi, "<em>"),
		...
		_shortcode2html: function (s) {
			...
			if (s.tag === 'row') {
				...
				return '<div class="mom-su-row' + cssclass + '">' + s.content + '</div>';
			}
			// Columns
				...
				return '<div class="mom-su-column mom-su-column-size-' + size + cssclass + '">' + s.content + '</div>';
			}
		}
	}),
