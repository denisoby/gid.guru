...
class null_instagram_widget extends WP_Widget {
	...
	function null_instagram_widget() {
		$widget_ops = array( 'classname' => 'momizat-instagram', 'description' => __( 'Displays your latest Instagram photos', 'wpiw' ) );
		$this->WP_Widget( 'momizat-instagram', __( 'Instagram', 'wpiw' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( $link != '' ) { 
				$link = ' @ <a href="//instagram.com/'.trim($username).'" rel="me" target="'.esc_attr( $target ).'">'.$link.'</a>';
		}

		echo $before_widget;
		if ( !empty( $title ) ) { echo $before_title . $title.$link . $after_title; };
			...
			if ( is_wp_error( $media_array ) ) {
				...
				foreach ( $media_array as $item ) {
					...
					if ( locate_template( 'parts/wp-instagram-widget.php' ) != '' ) {
						...
						echo '<li class="'. $liclass .'"><a href="'. esc_url( $item['link'] ) .'" target="'. esc_attr( $target ) .'"  class="'. $aclass .'"><img src="'. esc_url( $item['thumbnail'] ) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"  class="'. $imgclass .'" width="640" height="640" /></a></li>';
					}
				}
				?></ul><?php
			}
