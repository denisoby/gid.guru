...
'zu' => 'Zulu',
	...
	function mom_googleplus_Widget()
	{
		...
		$this->WP_Widget('momizat-googlebadgebox', __('Themelions - Google+ Badge Box','theme'), $widget_ops);
	}
...
<div class="mom-googleplus-widget">
	<div class="mgw-inner">
		<?php
		if($page_url): ?>	
			<div <?php if($page_type == 'profile') { ?>class="g-person"<?php } elseif($page_type == 'page') { ?>class="g-page"<?php } elseif($page_type == 'community') { ?>class="g-community"<?php } ?> data-width="<?php echo esc_attr($width); ?>" data-href="<?php echo esc_ulr($page_url); ?>" data-layout="<?php echo esc_attr($gp_layout); ?>" data-theme="<?php echo esc_attr($color_scheme); ?>" data-rel="publisher" data-showtagline="<?php echo esc_attr($tagline); ?>" data-showcoverphoto="<?php echo esc_attr($cover_photo); ?>"></div>
...
<div class="mgw-cover"></div>
	...
	{
		$defaults = array('title' => __('Google+','theme'), 'page_url' => '', 'width' => '274', 'color_scheme' => 'light', 'gp_layout' => 'portrait', 'page_type' => 'profile', 'cover_photo' => 'on', 'tagline' => 'on', 'lang' => '');
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('page_type')); ?>"><?php _e('Page type','theme'); ?>:</label> 
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('page_url')); ?>"><?php _e('Google+ Page URL','theme'); ?>:</label>
			<input type="text" class="widefat" id="<?php echo esc_attr($this->get_field_id('page_url')); ?>" name="<?php echo esc_attr($this->get_field_name('page_url')); ?>" value="<?php echo esc_attr($instance['page_url']); ?>" />
		</p>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('color_scheme')); ?>"><?php _e('Color Scheme','theme'); ?>:</label> 
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('gp_layout')); ?>"><?php _e('Layout','theme'); ?>:</label> 
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('cover_photo')); ?>"><?php _e('Cover Photo','theme'); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('tagline')); ?>"><?php _e('Tagline','theme'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('lang')); ?>"><?php _e('Language','theme'); ?>:</label> 
