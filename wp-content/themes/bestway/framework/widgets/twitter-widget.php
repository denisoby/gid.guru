...
class mom_widget_twitter extends WP_Widget {
	function mom_widget_twitter() {
		...
		$this->WP_Widget('momizatTwitter',__('Themelions - Twitter','theme'),$widget_ops);
	...
	<?php if ($style == 'minimum') { ?>
		<div class="twitter-widget">
				...
				foreach($tweets as $tweet) {
					$txt = isset($tweet['text']) ? $this->mom_tweets_hyperlinks($tweet['text']) : '';
					$txt = $this->mom_tweets_twitter_users($txt);
					$txt = $this->mom_tweets_encode_tweet($txt);

				?>
					<li><div><?php echo balanceTags($txt); ?></div></li>
				<?php
				if (++$i == $count) break;
	...
	<?php } else { ?>
			<ul class="twiter-list">
				...
				?>
					...
					<div class="tl-head">
                                   <a target="_blank" href="https://twitter.com/<?php echo esc_attr($tweet['user']['screen_name']); ?>"><img src="<?php echo esc_url($tweet['user']['profile_image_url']); ?>" alt=""></a>
                                   <span class="twitter-user-name"><a target="_blank" href="https://twitter.com/<?php echo esc_url($tweet['user']['screen_name']); ?>">@<?php echo esc_html($tweet['user']['screen_name']); ?></a><span class="date"><?php echo human_time_diff( strtotime($tweet['created_at']), current_time('timestamp') );  _e(' ago', 'theme'); ?></span>
</span>
                                   <a class="twiter-follow" href="https://twitter.com/intent/follow?screen_name=<?php echo esc_attr($tweet['user']['screen_name']); ?>" onclick="window.open(this.href, '', 'menubar=no,toolbar=no,resizable=no,scrollbars=no,height=455,width=600');return false"><?php _e('Follow', 'theme'); ?></a>
					</div> <!--head-->
                                   <div class="twiter-tweet">
					<?php echo balanceTags($txt); ?>
				   </div>
...
function form( $instance ) {
		...
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'username' )); ?>"><?php _e('Twitter Name:','theme'); ?></label>
		...
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'count' )); ?>"><?php _e('Number of tweets:','theme'); ?></label>
