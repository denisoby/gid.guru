...
function end_el( &$output, $item, $depth = 0, $args = array() ) {
        ...
        for ($i=0; $i<count($item->children);$i++) {
            ...
            $output .="<div class='".(($i===0)?'active':'')." mom-cat-latest' id='mom-mega-cat-".$child->ID."'>";
            $output .="<ul id='mom-mega-cat-".$child->ID."'>";
                if($child->object == 'category'){
                    ...
                    $r = new WP_Query( apply_filters( 'widget_posts_args', 
                      ...
                      if ($r->have_posts()) :
                            while ( $r->have_posts() ) {
                                    $r->the_post();
                            $output.= "<li><figure><a href='".get_permalink()."' title='".get_the_title()."'><img src='".mom_post_image($img_size)."' alt=''></a></figure><h2><a href='".get_permalink()."' title='".get_the_title()."'> ".get_the_title()."</a></h2></li>";
                                } 
