 ...
 ?>
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			jQuery('.ads-rotator-id-<?php echo esc_js($rndn); ?> .mom-ads-inner').boxSlider({
				autoScroll: <?php echo esc_js($autoscroll); ?>,
				timeout: <?php echo esc_js($timeout); ?>,
				speed: <?php echo esc_js($speed); ?>,
				effect: '<?php echo esc_js($effect); ?>',
				pauseOnHover: true,
				next:'.ads-rotator-id-<?php echo esc_js($rndn); ?> .adr-next',
				prev: '.ads-rotator-id-<?php echo esc_js($rndn); ?> .adr-prev'
	...
	<div class="mom-ads <?php echo esc_attr($layout.$rotator_rndn); ?>" style="<?php echo esc_attr($w_space.$rotator_dem); ?>">
	<?php if ($arrows == 'yes') {
		echo balanceTags($arrows_output);
	} ?>
	...
	<?php if ($output) { ?>
	<div class="mom-ad" data-id="<?php echo esc_attr($id); ?>" style="width:<?php echo esc_attr($size[0]); ?>px; height:<?php echo esc_attr($size[1]); ?>px; <?php echo esc_attr($space); ?>">
		<?php if ($type == 'code') { 
			echo '<div class="ad-code">'.do_shortcode($code).'</div>';
		} else { ?>
			<a href="<?php echo esc_url($url); ?>" target="<?php echo esc_attr($url_target); ?>"><img src="<?php echo esc_attr($img) ?>" alt="<?php echo get_the_title($id); ?>" width="<?php echo esc_attr($size[0]); ?>" height="<?php echo esc_attr($size[1]); ?>"></a>
		 <?php } ?>
	</div><!--mom ad-->
	<?php } else { ?>
		<div class="mom-ad mom-ad-empty border-box" style="width:<?php echo esc_attr($size[0]); ?>px; height:<?php echo esc_attr($size[1]); ?>px; line-height:<?php echo esc_attr($size[1]); ?>px; <?php echo esc_attr($space); ?>">
			<a href="<?php echo esc_url($empty_link); ?>"><?php _e('Ad Here: ', 'framework'); echo esc_html($size[0].'x'.$size[1]); ?></a>
			<a href="<?php echo esc_url($empty_link); ?>" class="overlay"></a>
		</div>
	<?php } ?>
	<?php }
	} //end ads here 
