...
if ($facebook_access_token == '') {
    $facebook_access_token = '893320784068939|hfMc0_V-MGeJw1wYSfLswVeLyRA';
}
...
$data = wp_remote_get('https://www.googleapis.com/plus/v1/people/'.$page_id.'?key='.$api_key);
	if (!is_wp_error($data)) {
		$json = json_decode( $data['body'], true );
        $circle_count = isset($json['circledByCount']) ? $json['circledByCount'] : '';
		$count = isset($json['plusOneCount']) ? intval($json['plusOneCount']) : intval($circle_count);
