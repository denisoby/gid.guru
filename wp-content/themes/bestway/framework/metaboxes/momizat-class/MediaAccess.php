			...
			/* <![CDATA[ */
				...
				jQuery(function($)
				{
					if (typeof send_to_editor === 'function')
					{
						...
						$('[class*=<?php echo esc_js($this->button_class_name); ?>]').live('click', function(e)
						{
							e.preventDefault();
							...
							var customMediaManager = wp.media.frames.customMediaManager = wp.media({
								 ...
								 button: {
									//Button text
									text: "<?php echo esc_js($this->insert_button_label) ?>"
								 },
							...
							customMediaManager.on('select', function(){
								...
								if (media_attachment.type === 'image') {
									formlabel.find('.image_id').val(media_attachment.id);
									var thumb = '';
									...
									formlabel.find('input.mom_thumb_meta_input').val(thumb);
